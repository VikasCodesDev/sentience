...
export default function CommandBar({ onSubmit, onFileAttached, pendingFileName, loading }: Props) {
  ...
  const displayFileName = file?.name || pendingFileName;
    ...
    >
      ...
      {displayFileName && (
        <div className="flex items-center gap-2 px-2 py-1.5 bg-cyan-500/10 border border-cyan-400/20 rounded-lg">
          ...
          <span className="text-xs text-cyan-300 font-mono flex-1 truncate">{displayFileName}</span>
          <button
      ...
      <div className="flex gap-2 items-center">
        ...
        <button
          ...
          className="p-2 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-400/20 rounded-lg transition-all duration-200 hover:border-cyan-400/40 hover:shadow-[0_0_10px_rgba(0,255,255,0.2)] text-base"
        ...
        <input
          ...
          placeholder={
            isListening
              ? "🎙 Listening..."
              : displayFileName
              ? "Add a message about the file (or press send)..."
        ...
        {recognition && (
          <button
            onClick={toggleVoice}
            className={`p-2 rounded-lg border transition-all duration-200 text-base ${
              isListening
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg border font-mono text-xs font-semibold transition-all duration-200 ${
            hasContent && !loading
